...
float computeOnDevice(float* h_data, int num_elements)
{
    ...
    // Allocate h_data on the device
    int size = num_elements * sizeof(float);
    float* A_d;
    cudaMalloc((void**)&A_d, size);

    // Load h_data from host to device
    cudaMemcpy(A_d, h_data, size, cudaMemcpyHostToDevice);
    ...
    reduction<<<DimGrid,DimBlock>>>(A_d, num_elements);
    reduction<<<1,DimBlock>>>(A_d, (num_elements - 1)/BLOCK_SIZE + 1);


    // Read h from device
    cudaMemcpy(h_data, A_d, size, cudaMemcpyDeviceToHost);
