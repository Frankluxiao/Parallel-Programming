...
/*  
    ...
    uint32_t* D_input = (uint32_t*)AllocDev(INPUT_SIZE * sizeof(uint32_t));
    uint8_t* D_bins = (uint8_t*)AllocDev(HISTO_SIZE * sizeof(uint8_t));
    uint32_t* G_bins = (uint32_t*)AllocDev(HISTO_SIZE * sizeof(uint32_t));
    CpyToDev(D_input, &(input[0][0]), INPUT_SIZE * sizeof(uint32_t));
*/

  
    int INPUT_SIZE_OPT = INPUT_HEIGHT*((INPUT_WIDTH + 128) & 0xFFFFFF80);
    int HISTO_SIZE = HISTO_HEIGHT*HISTO_WIDTH;
    uint32_t* D_input = (uint32_t*)AllocDev(INPUT_SIZE_OPT * sizeof(uint32_t));
    uint8_t* D_bins = (uint8_t*)AllocDev(HISTO_SIZE * sizeof(uint8_t));
    uint32_t* G_bins = (uint32_t*)AllocDev(HISTO_SIZE * sizeof(uint32_t));
    CpyToDev(D_input, &(input[0][0]), INPUT_SIZE_OPT * sizeof(uint32_t));
    ...
    CpyFromDev(kernel_bins, D_bins, HISTO_HEIGHT * HISTO_WIDTH * sizeof(uint8_t));
    FreeDev(D_bins);
    FreeDev(D_input);
    FreeDev(G_bins);
